...
typedef struct
{
    ...
    float err;             			//Define error value
    float err_last;          		//Define previous error value
	float err_next;
    float Kp,Ki,Kd;          		//Define proportional, integral, derivative coefficients

	float integral;          		//Define integral value (position PID)
	float I_Control;
	float absmax;
	float absmin;
}_pid;
