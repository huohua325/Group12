 ...
 * 2. Encoder speed acquisition: real-time reading of wheel speed with filtering
 * 
 ...
 * @note Initialize GPIO and PWM channels, motor defaults to stopped state
 * @note Must be called before using any motor control functions
 */
void Motor_Init(TIM_HandleTypeDef *htim);

/**
 * @brief Set speed and direction of a single motor (dual PWM H-bridge drive) ⭐
...
// ============================================================================
// Legacy open-loop control functions have been completely removed
// ============================================================================
...
//   4. Use Car_Stop_PID() to stop motor

/**
 * @brief Motor control system initialization (with speed loop PID)
 * @note Initialize control parameters and PID controller, must be called before use
 */
void Motor_PID_Init(void);

/**
 * @brief Dual motor speed loop PID control cycle ⭐
 ...
 * @note Uses PID_realize() function in pid.c to implement independent speed loop control for dual motors
 * @note Must be continuously called in main while loop (internal 20ms timing)
 * @example
 *   while(1) {
...
/**
 * @brief Set left/right wheel base target speed (before angle loop superposition) ⭐
 ...
 * @note Angle loop will automatically select shortest path turning, dead zone 1.5 degrees
 * @note Angle loop output will be superimposed on speed loop target to achieve differential steering
 * @example
 ...
 * @param target_pos Target position (cm, relative to position zero point)
 * @note Position loop will automatically control vehicle to move to target position, dead zone 0.5cm
 * @note Use with Motor_ResetPosition(), clear position first then set target
 * @note Position calculated based on average of two wheel encoders, tire diameter 75mm
 * @example
 *   Motor_ResetPosition();          // Clear position
 *   Motor_SetTargetPosition(50.0f); // Move forward 50cm
 *   Motor_SetTargetPosition(-30.0f); // Move backward 30cm (relative to zero point)
 */
void Motor_SetTargetPosition(float target_pos);

/**
 * @brief Get current accumulated position
 * @return Current position (cm)
 */
...
/**
 * @brief Get currently set target speed
 ...
 * @note Call when motor changes direction or oscillates to prevent integral saturation
 * @note Will clear integral terms, error history and PWM output of position loop, angle loop, speed loop
 * @example
...
/**
 * @brief Get actual speed (filtered speed values)
 ...
 * @note Also clears target position and position loop PID state
 * @note Must be called before using Motor_SetTargetPosition()
 * @example
 *   Motor_ResetPosition();          // Clear position
 *   Motor_SetTargetPosition(50.0f); // Move forward 50cm from current position
 */
 ...
 * @note Factor range limited between 0.5~1.5
 * @example
 ...
 * @note Factor range limited between 0.9~1.1
 * @example
...
/**
 * @brief Gyroscope angle calibration assistant (automatically calculate calibration factor) ⭐
 ...
 * @note Used for debugging, this is the normalized angle directly output by gyroscope
 */
 ...
 * @note This is the actual angle value used
 */
 ...
 * @note Used to check how many degrees the gyroscope has accumulated (uncalibrated)
 * @example
 ...
 * @note Used to check how many degrees accumulated after calibration
 * @example
 *   // After turning 10 circles
 *   float acc = Motor_GetAccumulatedCalibratedAngle();
 *   printf("Calibrated accumulation: %.1f degrees\n", acc);  // Should show 3600 degrees
 */
 ...
 * @note Call when re-calibrating or need to clear accumulated angle
 * @example
 ...
 * @param distance_cm Forward distance (cm, positive=forward, negative=backward)
 * @param timeout_ms Timeout time (ms, reserved parameter but not used)
 ...
 * @note When target reached (position dead zone 0.5cm + angle dead zone 1.5 degrees) will send character 'Z' via UART4
 * @example
...
/**
 * @brief Command: Turn left in place by specified angle (non-blocking) ⭐
 * @param angle_deg Rotation angle (degrees, positive=counterclockwise left turn, negative=clockwise right turn)
 * @param timeout_ms Timeout time (ms, reserved parameter but not used)
 * @return 0=command set
 * @note Non-blocking execution, returns immediately after setting target
 * @note Need to continuously call Motor_PID_Control() in main loop to execute control
 * @note When target reached (angle dead zone 1.5 degrees) will send character 'Z' via UART4
 * @example
...
/**
 * @brief Command: Turn right in place by specified angle (non-blocking) ⭐
